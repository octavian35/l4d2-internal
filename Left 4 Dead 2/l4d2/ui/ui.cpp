...
void ui::run()
{
	...
	ImGui::Begin(X_("Left 4 Dead 2"), &m_opened, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);
	{
		...
		ImGui::MakePopup(X_("aimbot"), &g_var.m_aim_bot_var, []()
		{
			ImGui::Checkbox(X_("auto fire"), &g_var.m_aim_fire_var);
			ImGui::Checkbox(X_("recoil compensation"), &g_var.m_aim_compensation_var);
			ImGui::SliderFloat(X_("fov"), &g_var.m_aim_fov_var, 0.0f, 180.0f, X_("%.0fm"));
		});

		ImGui::Checkbox(X_("anti aim"), &g_var.m_anti_aim_var);
