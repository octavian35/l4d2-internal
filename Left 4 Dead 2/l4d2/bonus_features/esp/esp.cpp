...
static void draw_infected(c_base_player* entity)
{
	vec3 entity_position, entity_origin = entity->origin();


	c_client_class* cls = entity->client_class();
	if (!cls || cls->class_id != ET_INFECTED)
		return;
	...
	g_render.draw_text(
		...
		entity_position.y,
		g_fonts.m_font,
		g_render.create_col(g_var.m_esp_infected_color_var),
		true, false
	);
}
